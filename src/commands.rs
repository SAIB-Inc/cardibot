...
pub async fn handle_issue_command(
    ...
    if let Some(required_role_id) = &project.allowed_role_id {
        ...
        let required_role_id = required_role_id.parse::<u64>()
            .map_err(|_| "Invalid role ID in configuration")?;
        
        let has_role = member.roles.iter().any(|role_id| role_id.get() == required_role_id);
    ...
    let action = if result.was_updated { "Updated" } else { "Created" };
    tracing::info!(
